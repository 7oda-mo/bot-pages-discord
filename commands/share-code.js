...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("code").setDescription("الكود البرمجي").setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("version")
    ...
    .addStringOption((option) =>
      option.setName("name").setDescription("اسم الكود").setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("description")
    ...
    .addStringOption((option) =>
      option
        ...
        .addChoice("JavaScript", "Discord.js") ///name = value
        .addChoice("Python", "Discord.py") ///name = value
        .setRequired(true)
    )
    .addChannelOption((option) =>
      option.setName("channel").setDescription("القناة التي تريد المشاركة فيها")
    ),
  async execute(interaction) {
    ...
    if (channelOption) {
      ...
      await interaction.reply(
        `تم أرسال الكود في القناة <#${targetChannel.id}>. شكراً على المشاركة.`
      );
    } else {
      targetChannel = interaction.channel;
      await interaction.reply(
        `تم أرسال الكود في القناة الحالية. شكراً على المشاركة.`
      );
    }
    ...
    const embed = new EmbedBuilder()
      .setDescription("```js\n" + code + "```")
      .setAuthor({ name: `${type} Code`, iconUrl: iconUrl })
      .addFields(
      ...
      .setFooter({
        text: `تم برمجة هذا البوت بواسطة ${developer.tag}`,
    ...
    collector.on("collect", async (i) => {
      ...
      try {
        ...
        await i.reply({
          content: "تم إرسال الرمز إلي رسائلك الخاصة.",
          ephemeral: true,
        });
      } catch (error) {
        console.error("حدثت مشكلة أثناء إرسال الكود في الرسائل الخاصة:", error);
        await i.reply({
          content: "حدثت مشكلة أثناء إرسال الرمز في الرسائل الخاصة.",
          ephemeral: true,
        });
      }
    });
